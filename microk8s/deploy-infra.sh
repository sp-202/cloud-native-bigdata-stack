...
# MicroK8s Check
if kubectl get nodes -o wide | grep -q "microk8s"; then
    echo "Detected MicroK8s cluster."
    # Check if MetalLB is enabled (LoadBalancers won't work without it)
    if ! microk8s status | grep -q "metallb: enabled"; then
         echo "WARNING: MetalLB is NOT enabled."
         echo "Your LoadBalancer Services (Traefik, Zeppelin) will hang in 'Pending'."
         echo "Please run: microk8s enable metallb"
...
helm upgrade --install kube-prometheus-stack prometheus-community/kube-prometheus-stack --namespace default --set grafana.adminPassword=prom-operator --set prometheus.prometheusSpec.podMonitorSelectorNilUsesHelmValues=false --set prometheus.prometheusSpec.serviceMonitorSelectorNilUsesHelmValues=false --set prometheus.prometheusSpec.storageSpec.volumeClaimTemplate.spec.storageClassName=microk8s-hostpath --set prometheus.prometheusSpec.storageSpec.volumeClaimTemplate.spec.accessModes[0]=ReadWriteOnce --set prometheus.prometheusSpec.storageSpec.volumeClaimTemplate.spec.resources.requests.storage=10Gi --set prometheus.prometheusSpec.retention=168h
