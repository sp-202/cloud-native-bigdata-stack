...
# Traefik (K3s Bundled)
echo "Skipping Traefik Install (Using K3s Default)..."

# Wait for Traefik LoadBalancer IP (K3s usually runs it in kube-system as traefik, or default svclb)
echo "Waiting for Traefik LoadBalancer IP..."
EXTERNAL_IP=""
while [ -z "$EXTERNAL_IP" ]; do
  # Check kube-system (default k3s location)
  EXTERNAL_IP=$(kubectl get svc -n kube-system traefik -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null)
  
  # Fallback: Check default namespace
  if [ -z "$EXTERNAL_IP" ]; then
      EXTERNAL_IP=$(kubectl get svc traefik -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null)
  fi

  # Fallback for K3s VM (use Node IP if LB is pending)
  if [ -z "$EXTERNAL_IP" ]; then
       # Assume Node Port or Host Network if LB is stuck (common in simple K3s)
       echo "LoadBalancer IP pending... trying Node IP..."
  ...
  if [ -z "$EXTERNAL_IP" ]; then
    ...
    # After 30s timeout, assume user might need to input it (common on some VMs)
    counter=$((counter+1))
    if [ $counter -gt 3 ]; then
        echo "Could not auto-detect LoadBalancer IP."
        echo "Please enter the K3s Node IP or LoadBalancer IP manually:"
        read EXTERNAL_IP
    fi
  fi
done
echo "Cluster IP/LB Assigned: $EXTERNAL_IP"
...
helm upgrade --install kube-prometheus-stack prometheus-community/kube-prometheus-stack --namespace default --set grafana.adminPassword=prom-operator --set prometheus.prometheusSpec.podMonitorSelectorNilUsesHelmValues=false --set prometheus.prometheusSpec.serviceMonitorSelectorNilUsesHelmValues=false --set prometheus.prometheusSpec.storageSpec.volumeClaimTemplate.spec.storageClassName=local-path --set prometheus.prometheusSpec.storageSpec.volumeClaimTemplate.spec.accessModes[0]=ReadWriteOnce --set prometheus.prometheusSpec.storageSpec.volumeClaimTemplate.spec.resources.requests.storage=10Gi --set prometheus.prometheusSpec.retention=168h
